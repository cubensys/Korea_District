...
- 공간데이터 분석에서, 특정 지역간에 어떤 값(Value)을 비교하거나 값의 분포를 파악하는 일은  많이 사용하는 방법중 하나입니다.   
	*예1)  강남구와 강북구의 연간 예산 비교*  
    *예2) 서울시의 공공자전거 대여소 분포 현황*

- 이러한 분석을 위해서는 의미있는 단위로 전체 공간을 분할하여 구분해야 합니다.  
    *예1) 서울전역을 100미터 단위의 셀로 분할*  
    *예2) 대한민국 전역을 기초자치단체 행정구역별로 분할*

- 이 곳에서는 공간분석 도구에서 활용할 수 있도록, 대한민국과 서울시를 행정구역 단위로 나눈 파일을 관리합니다.
